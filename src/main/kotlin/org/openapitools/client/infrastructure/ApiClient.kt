...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["x-store-key"].isNullOrEmpty()) {
            if (apiKey["x-store-key"] != null) {
                if (apiKeyPrefix["x-store-key"] != null) {
                    requestConfig.headers["x-store-key"] = apiKeyPrefix["x-store-key"]!! + " " + apiKey["x-store-key"]!!
                } else {
                    requestConfig.headers["x-store-key"] = apiKey["x-store-key"]!!
                }
            }
        }
    }
